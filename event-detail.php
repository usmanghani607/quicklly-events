...
if ($result_home['success'] && isset($result_home['lstProds'])) {
    ...
    if ($result['success']) {
        ...
        // $api_url_lock_qty = 'https://devrestapi.goquicklly.com/events/lock-qty';
        // $data_lock_qty = array(
        //     "eid" => $event_id,
        //     "uid" => $uid,
        //     "lstSizes" => $lstSizes
        ...
        // $ch = curl_init($api_url_lock_qty);
        // curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        // curl_setopt($ch, CURLOPT_POST, true);
        // curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($data_lock_qty));
        // curl_setopt($ch, CURLOPT_HTTPHEADER, array(
        //     'Content-Type: application/json',
        //     'Authorization: Bearer ' . $bearer_token
        // ));

        // $response_lock_qty = curl_exec($ch);
        // curl_close($ch);

        // $result_lock_qty = json_decode($response_lock_qty, true);

        // if ($result_lock_qty['success'] && isset($result_lock_qty['cartID'])) {
        //     $cartID = $result_lock_qty['cartID'];

        //     $_SESSION['cartID'] = $cartID;
        //     // echo "Cart ID: " . htmlspecialchars($cartID);
        // } else {
        //     echo "Failed to retrieve cart ID.";
        // }
    } else {
        echo "Event details not found.";
    }
...
<head>
    ...
    <style>
        ...
        /* .event-detail .side-area.side-fixed {
            position: fixed;
            z-index: 1;
        } */

        /* .event-detail .summery-p {
            position: fixed;
            width: 312px;
            z-index: 111;
        } */

        /* .event-detail .social-p {
            position: fixed;
            width: 312px;
            top: 66%;
            z-index: 111;
